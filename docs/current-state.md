...
D22 신규 Electron 실물 검사와 RPG ASAR opt-in 계약 수정을 완료했다.
현재 기준은 `main@a9690a6` 위 **미커밋 로컬 변경**이다.
...
다음은 실행 프로브 사용자 프로필 격리와 실패 작업팩 01·11·14·16 대조다.
게임 실행·의미 감수는 미실행이다. 기존 배포 ZIP에는 이번 소스 수정이 포함되지 않는다.
