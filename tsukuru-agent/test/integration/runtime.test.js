...
const portablePeFixture = path.join(appRoot, 'exfiles', 'eztrans', 'EztransServer.exe');
let electronExecutable = null;
const electronPackageRoot = path.dirname(require.resolve('electron/package.json'));
if (fs.existsSync(path.join(electronPackageRoot, 'path.txt'))) {
...
const {
    ...
    runLaunchProbe,
} = require('../../.build/app/src/core/runtimeDiagnostics.js');
...
test('reads the packaged Electron fuse wire without modifying the executable', { skip: !electronExecutable }, async () => {
...
test('reports a missing Windows ElectronAsar integrity resource separately from ASAR file integrity', async () => {
    ...
    const result = inspectWindowsAsarIntegrity(portablePeFixture, archive, 'resources/app.asar');
...
test('distinguishes a matching embedded ElectronAsar resource from a stale one after repack', async () => {
    ...
    const executable = NtExecutable.from(fs.readFileSync(portablePeFixture), { ignoreCert: true });
...
test('inspects the Windows executable signature without modifying the file', () => {
    const executable = portablePeFixture;
    const before = fs.statSync(executable);
...
test('preserves non-ASCII executable paths when requesting Authenticode status', { skip: process.platform !== 'win32' }, () => {
...
test('launch probe tolerates exit-event delivery after Windows tree termination returns', { skip: process.platform !== 'win32' }, async () => {
    const originalSpawnSync = childProcess.spawnSync;
    let interceptedTreeKill = false;
    childProcess.spawnSync = (command, args, options) => {
        const result = originalSpawnSync(command, args, options);
        if (command !== 'taskkill') return result;
        interceptedTreeKill = true;
        return { ...result, status: result.status === 0 ? 1 : result.status };
    };

    let result;
    try {
        result = await runLaunchProbe(
            process.execPath,
            ['-e', 'setInterval(() => {}, 1000)'],
            { timeoutMs: 150 },
        );
    ...
    assert.equal(interceptedTreeKill, true);
    assert.equal(result.status, 'running');
    assert.equal(result.timedOut, true);
    assert.equal(result.terminatedByProbe, true);
});
...
test('aggregates fuse, ASAR resource, and signature diagnostics for an Electron root', async () => {
    ...
    fs.mkdirSync(source);
    fs.writeFileSync(path.join(source, 'package.json'), '{}');
    fs.copyFileSync(portablePeFixture, path.join(root, 'Game.exe'));
    const stream = await asar.createPackage(source, archive);
    if (!stream.writableFinished) await finished(stream);

    const result = await inspectElectronRuntime(root, archive, 'resources/app.asar');
