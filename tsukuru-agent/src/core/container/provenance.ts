...
export function writeContainerProvenance(root: string, provenance: ContainerProvenance): string {
    const contract = validateContract('container-provenance', PROVENANCE_SCHEMA_VERSION, provenance);
    if (!contract.ok) {
        throw invalid('컨테이너 provenance 스키마 검증에 실패했습니다', { violations: contract.errors });
    }
...
export function readContainerProvenance(root: string): ContainerProvenance | null {
    ...
    try {
        ...
        throw invalid('컨테이너 provenance JSON을 읽을 수 없습니다', { path: target, cause: String(error) });
    }
    ...
    const contract = validateContract('container-provenance', PROVENANCE_SCHEMA_VERSION, value);
    if (!contract.ok) {
        throw invalid('컨테이너 provenance 스키마 검증에 실패했습니다', { violations: contract.errors });
    }
