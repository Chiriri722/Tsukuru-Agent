...
> **현재 계획 + 구현 이력:** 맨 위 D22는 2026-09-23 실물 검증을 반영합니다. D21과 초기 개조 이력은 보존하며, 장기 단계와 Exit Gate는 `New-task-plan.md`에 연결합니다.

## 현재 계획 — D22 Electron 실물 추가 검증 (2026-09-23)

[검증 보고서](docs/reviews/2026-09-23-electron-corpus.md)와 003의 T022–T025에 근거를 남겼다.
이번 수정은 `main@a9690a6` 위 로컬 변경이며 아직 커밋·패키징하지 않았다.

- [x] **D22-01 — 신규 Electron 샘플 추가.** 별도 복사본에서 19,465항목 추출·deep verify와
  검증용 제목 1건의 사전 apply/repack을 통과했다. 유효 payload 3,994개 중 의도한 JSON 1개만 변경됐다.
- [x] **D22-02 — RPG ASAR 옵션 계약 수정.** 기존 재포장 opt-in을 거부하던 RPG 스키마 누락을
  RED/GREEN으로 수정했다. verify/order 각각 433/433, benchmark 6개와 실제 샘플 재실행 통과.
- [x] **D22-03 — 이동·수리 작업팩 대조.** 기존 14개 위치 확인, 변경 5개·신규 1개 검사:
  5개 통과·1개 실패. 기존 실패 3개가 통과로 바뀌었지만 15는 원문 유지 상태다.
  원본 게임·복사본·추출 작업본·실제 AppData 불변과 세이브 ZIP 보존을 확인했다.
- [ ] **D22-04 / P0 — 실행 프로브의 사용자 프로필 격리.** 게임 복사만으로 격리되지 않는
  Electron AppData 쓰기를 synthetic fixture/canary로 재현하고, 지원 wrapper의 격리와 정리까지 검증한다.
- [ ] **D22-05 / P1 — 남은 실패 작업팩 대조.** 01·11·14·16을 원문 ID·Backup 기준으로 확인한다.
  신규 16의 해시 불일치 12,776개와 제어코드 손상을 우선 조사하고 recover와 의미 복구를 구분한다.
- [ ] **D22-06 / P1 — 격리 후 실게임 확인.** 신규 샘플의 기동·저장/불러오기·대표 장면을
  검증한다. 현재 실행·의미 감수는 미실행이며 기존 수동 릴리스 gate도 남아 있다.

**이어갈 순서:** D22-04 → D22-05 → D22-06. 검증 출력은 전체 한글 패치 배포본이 아니다.

## D21 완료 기록 — Daybreak 후속 검증 (2026-09-13)
...
실제 작업팩 14개(522,620항목)는 8개 통과·6개 오류 검출이며 원본/복사본 해시는 유지됐다.
최종 코드로 JSON 오류·정상·번역 손상 3개 사례를 재검증했다. 의미 감수·실게임은 미실행이다.
...
- 보고서는 main `8c7d773`의 78개 로컬 테스트를 기준으로 했다.
  ...
  당시 일반 경로의 main에는 이 구현이 병합되지 않은 상태였다.
...
공식 Spec-kit으로 003 명세/설계를 구체화했고 기존 002 완료 기록을 보존했다.
