...
- [x] 깨끗한 체크아웃에서 `npm ci && npm run verify && npm run build:cli`가 한 절차로 완료된다.
...
- [x] GUI main thread에서 장시간 변환을 분리한다.
...
- [x] GUI가 장시간 동기 작업으로 응답 불능이 되지 않는다.
...
- [!] 승인된 실제 directory-form 샘플이 corpus에 없어 launch probe·playtest gate는 미확인이다.
...
Phase 0에서 답을 확정하고 `Decisions Made`로 이동한다.

1. GUI는 장기적으로 핵심 제품인가, 호환성 유지 대상인가?
2. `exfiles/**` 번역 엔진은 기본 GUI 배포에 반드시 포함되어야 하는가?
3. 공식 지원 Node/npm·Windows 범위는 무엇인가?
4. README의 과거 smoke 테스트는 Git 이력에서 복구할 가치가 있는가, 현재 v25 suite로 대체되었는가?
5. 다음 릴리스는 호환 patch/minor인가, schema·packaging 변경을 포함한 major인가?
6. 실제 MV/MZ·Wolf·Tyrano·GDevelop 샘플 corpus를 어떤 권한과 저장 방식으로 운영할 것인가?
7. GUI 자동 업데이트를 유지할 것인가, release 알림만 제공할 것인가?
8. 외부 번역 서비스와 네트워크 기능을 core 제품에서 계속 유지할 것인가?
9. macOS/Linux는 core/CLI 테스트 대상인가, 정식 배포 대상인가?
10. warning의 기존 문자열 배열을 언제까지 유지할 것인가?
...
| 2026-08-15 | 기존 `task_plan.md`는 역사 문서로 보존하고 새 `task-plan.md`를 후속 개선의 단일 작업 원장으로 사용한다. | 완료된 개조 이력과 앞으로의 작업을 혼합하지 않기 위함 |
...
1. 사용자 승인 후 현재 통합 변경을 한 커밋 단위로 정리하고 공개 branch에 푸시한다.
...
- [x] GUI와 CLI가 같은 application layer를 호출한다.
